...
import { Monitor, Users, BookOpen, User } from "lucide-react"
...
export default function ResearchRegisterPage() {
  ...
  const onSubmit = async (data: ResearchFormValues) => {
    ...
    if (!user || !user.userId) {
      setShowAuthPrompt(true)
      toast.error("Please sign in to register for research consultation")
      return
    }

    try {
      // Submit research profile
      const payload = {
        ...data,
        userId: user.userId,
      }
      ...
      if (response.ok) {
        console.log("Research registration successful:", payload)
        toast.success("Registration successful! Welcome to our research consultation program.")
    setSubmitted(true)
    reset()
        setTimeout(() => {
          setSubmitted(false)
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-orange-50">
      ...
      >
        ...
        <div className="relative max-w-7xl mx-auto min-h-[calc(50svh_-_7rem)] flex items-center">
          <h1 className="text-5xl font-bold">Research Consultation</h1>
        </div>
      </section>
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        <Card className="border-0 bg-white/80 backdrop-blur-sm">
          <CardHeader className="text-center pb-8">
            <CardTitle className="text-3xl font-bold text-gray-800">Research Consultation Services</CardTitle>
            <p className="text-gray-600 mt-2">Get expert guidance for your research projects</p>
          </CardHeader>
          <CardContent className="px-8 pb-8">
            {showAuthPrompt && (
              ...
              <div>
                    ...
                    <p className="text-sm text-gray-600">Please create an account or sign in to continue with research consultation registration.</p>
                  </div>
                </div>
              </div>
            )}
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
              ...
              <div>
                        ...
                        <p className="text-gray-600 mb-4">
                          You need to create an account or sign in to register for research consultation.
                        </p>
              ...
              <div className={`space-y-6 ${!user ? 'opacity-50 pointer-events-none' : ''}`}>
                <h3 className="text-xl font-semibold text-gray-800 border-b-2 border-[#FF6652]/20 pb-2">Research Profile</h3>
              ...
              <div className="space-y-4">
                <h3 className="text-xl font-semibold text-gray-800 border-b-2 border-[#FF6652]/20 pb-2">Research Information *</h3>
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Textarea 
                      {...register("studyArea")} 
                      rows={4} 
                      placeholder="Describe your research area, topic, or field of study" 
                      className="border-gray-300 focus:border-[#245D51] focus:ring-[#245D51]"
                    />
                    {errors.studyArea && <p className="text-sm text-[#FF6652]">{errors.studyArea.message}</p>}
                  </div>
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        <Select onValueChange={field.onChange} value={field.value}>
                          <SelectTrigger className="border-gray-300 focus:border-[#245D51] focus:ring-[#245D51]">
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="professional">Professional Research</SelectItem>
                          </SelectContent>
                        </Select>
                      )}
                    />
              ...
              {/* Delivery Method */}
              <div className="space-y-4">
                <h3 className="text-xl font-semibold text-gray-800 border-b-2 border-[#245D51]/20 pb-2">Consultation Method *</h3>
                <p className="text-sm text-gray-600">Choose how you prefer to receive research consultation</p>
                <Controller
                  name="deliveryMethod"
                  control={control}
                  render={({ field }) => (
