...
type EntrepreneurshipValues = z.infer<typeof EntrepreneurshipSchema>

type Props = { onBack: () => void }

export default function EntrepreneurshipRegistrationForm({ onBack }: Props) {
	...
	const form = useForm<EntrepreneurshipValues>({
		resolver: zodResolver(EntrepreneurshipSchema),
		mode: 'onSubmit',
		reValidateMode: 'onChange',
		defaultValues: { firstName: '', lastName: '', email: '', phone: '', address: '', password: '', confirmPassword: '', age: undefined, gender: undefined, deliveryMethod: undefined, instructorId: '' }
	})
	...
	const onSubmit = async (data: EntrepreneurshipValues) => {
		setLoading(true)
		try {
			...
			const dupRes = await fetch(`/api/entrepreneurship?userId=${userId}`)
			const dupData = await dupRes.json()
			if (dupRes.ok && dupData.success && Array.isArray(dupData.data) && dupData.data.length > 0) { toast.error('User is already registered for entrepreneurship'); setLoading(false); return }

			const payload = { userId, age: data.age, gender: data.gender, deliveryMethod: data.deliveryMethod, instructorId: data.instructorId || undefined }
			const resp = await fetch('/api/entrepreneurship', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
			const result = await resp.json()
			if (!resp.ok || !result.success) {
				if (result?.details && Array.isArray(result.details)) { result.details.forEach((err: any) => { if (err?.message) toast.error(err.message) }) } else { toast.error(result.error || 'Failed to register for entrepreneurship') }
				setLoading(false)
				return
			}

			toast.success('Entrepreneurship registration completed successfully!')
			form.reset({ firstName: '', lastName: '', email: '', phone: '', address: '', password: '', confirmPassword: '', age: undefined, gender: undefined, deliveryMethod: undefined, instructorId: '' })
	...
	return (
		<div className="max-w-5xl">
			<Card className="bg-white">
				<CardHeader>
					<CardTitle className="text-gray-900">Entrepreneurship Registration</CardTitle>
				</CardHeader>
						...
						<form onSubmit={form.handleSubmit(onSubmit)}>
							<div className="space-y-4">
								...
								<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
									...
									<FormField control={form.control} name="lastName" render={({ field }) => (<FormItem className="space-y-2"><FormLabel className="text-gray-700 font-medium">Last Name</FormLabel><FormControl><Input className="bgwhite border-gray-300 text-gray-900" {...field} /></FormControl><FormMessage /></FormItem>)} />
