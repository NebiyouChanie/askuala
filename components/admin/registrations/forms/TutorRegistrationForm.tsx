...
const TutorSchema = z.object({
	...
	cv: z.any()
		.refine((file) => typeof File !== 'undefined' && file instanceof File, 'CV (PDF) is required')
		.refine((file) => !file || (file as File).type === 'application/pdf', 'PDF only')
		.refine((file) => !file || (file as File).size <= 5 * 1024 * 1024, 'Max 5MB'),
}).superRefine((val, ctx) => {
...
export default function TutorRegistrationForm({ onBack }: Props) {
	...
	const form = useForm<TutorValues>({
		...
		defaultValues: { firstName: '', lastName: '', email: '', phone: '', address: '', password: '', confirmPassword: '', age: undefined, gender: undefined, gradeLevels: [], subjects: [], startTime: '', endTime: '', availableDays: [], deliveryMethod: undefined, cv: null }
	})
	...
	const onSubmit = async (data: TutorValues) => {
		setLoading(true)
		try {
			...
			const dupRes = await fetch(`/api/tutors?userId=${userId}`)
			const dupData = await dupRes.json()
			if (dupRes.ok && dupData.success && Array.isArray(dupData.data) && dupData.data.length > 0) { toast.error('User is already registered as a tutor'); setLoading(false); return }

			let cvPath = ''
			if (data.cv) {
				const fileData = new FormData()
				fileData.append('cv', data.cv)
				const uploadResponse = await fetch('/api/tutors/upload-cv', { method: 'POST', body: fileData })
				const uploadData = await uploadResponse.json()
				if (!uploadResponse.ok || !uploadData.success) { toast.error(uploadData.error || 'Failed to upload CV'); setLoading(false); return }
				cvPath = uploadData.data.filePath
			}

			const payload = { userId, age: data.age, gender: data.gender, gradeLevels: data.gradeLevels, subjects: data.subjects, startTime: data.startTime, endTime: data.endTime, availableDays: data.availableDays, deliveryMethod: data.deliveryMethod === 'online-&-face-to-face' ? 'online-&-face-to-face' : data.deliveryMethod, cvPath }
			const resp = await fetch('/api/tutors', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
			const result = await resp.json()
			if (!resp.ok || !result.success) {
				if (result?.details && Array.isArray(result.details)) { result.details.forEach((err: any) => { if (err?.message) toast.error(err.message) }) } else { toast.error(result.error || 'Failed to register tutor') }
				setLoading(false)
				return
			}

			toast.success('Tutor registration completed successfully!')
			form.reset({ firstName: '', lastName: '', email: '', phone: '', address: '', password: '', confirmPassword: '', age: undefined, gender: undefined, gradeLevels: [], subjects: [], startTime: '', endTime: '', availableDays: [], deliveryMethod: undefined, cv: null })
	...
	return (
		<div className="max-w-5xl">
			<Card className="bg-white">
				<CardHeader>
					<CardTitle className="text-gray-900">Tutor Registration</CardTitle>
				</CardHeader>
					...
					<Form {...form}>
						<form onSubmit={form.handleSubmit(onSubmit)}>
							<div className="space-y-4">
								...
								<TutorFields form={form} gradeLevelRanges={gradeLevelRanges} subjects={subjects} daysOfWeek={daysOfWeek} />
